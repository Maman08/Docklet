...
// Submit task (updated to handle GitHub deploy)
router.post('/submit', 
    authenticateToken,
    upload.single('file'), 
    validateTask, 
    ...
    async (req, res) => {
        ...
        try {
            ...
            // GitHub deploy task doesn't need file upload
            ...
            // Validate GitHub URL for deploy task
            ...
            // File validation for file-based tasks
            if (file) {
                try {
                    await fs.access(file.path);
                    logger.info(`File uploaded successfully: ${file.path}, size: ${file.size} bytes`);
            ...
            // Upload file to S3 for file-based tasks
            if (file) {
                s3Key = s3Service.generateUserUploadKey(req.user.id, file.originalname);
                try {
                    await s3Service.uploadFile(file.path, s3Key, file.mimetype);
            ...
            // Add file info for file-based tasks
            if (file) {
                taskData.inputFile = {
                    originalName: file.originalname,
                    filename: file.filename,
            ...
            // Different response for GitHub deploy
            const response = {
                message: 'Task submitted successfully', 
                taskId,
...
// Get task status (updated to handle GitHub deploy)
router.get('/status/:taskId', async (req, res) => {
    try {
        const { taskId } = req.params;
        const task = await Task.findOne({ id: taskId });
        ...
        // Add different response data based on task type
...
// Download file (unchanged - only for file-based tasks)
router.get('/download/:taskId', async (req, res) => {
    try {
        const { taskId } = req.params;
        const task = await Task.findOne({ id: taskId });
...
// Get user profile with tasks (updated to handle GitHub deploy)
router.get('/profile/:userId', authenticateToken, async (req, res) => {
    try {
        const userId = req.params.userId;
        if (!userId) {
        ...
        return res.status(200).json({
            ...
            tasks: tasks.map(task => {
                ...
                // Add appropriate URL based on task type
                if (task.type === 'github-deploy') {
...
// Get running deployments (new endpoint)
